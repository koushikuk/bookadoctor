...
class AES {
    ...
    function __construct($data = null, $key = null, $blockSize = null, $mode = null) {
        ...
        $this->setMode($mode);
		$this->setIV("00000000000000000000000000000000");
    }
    ...
    public function encrypt() {
        ...
        if ($this->validateParams()) {
            /*  return trim(base64_encode(
                            mcrypt_encrypt(
                                    $this->cipher, $this->key, $this->data. str_repeat(chr($pad), $pad), $this->mode, $this->getIV()))); */
									
			$td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
			mcrypt_generic_init($td, $this->key, $this->hexToStr($this->getIV()));
    ...
    public function decrypt() {
        if ($this->validateParams()) {
			...
			$td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
			mcrypt_generic_init($td, $this->key, $this->hexToStr($this->getIV()));
			$data = mdecrypt_generic($td, base64_decode($this->data)); 
